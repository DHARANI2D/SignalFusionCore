...
1. **Change alert status**: Click "Start Triage" or "Close Alert" buttons in the Triage Controls panel
2. **View audit logs**: Check the "Recent History" section to see all status changes
3. **Execute remediation**: Use the Response Orchestration terminal

**Note**: The analyst notes feature (NoteForm component) is implemented in the codebase but not currently rendered in the UI. To enable it, the `<NoteForm alertId={alert.id} />` component needs to be added to the AlertDetail page layout.
